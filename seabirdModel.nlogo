...
turtles-own
[
  settled? ;Whether or not a breeding ground has been chosen
  ...
  mating-status? ;T/F of whether the bird is paired
  mating-id ;identifier for mating pairs
  breeding-status? ;Whether or not this bird has reached a breeding habitat
  last-breeding-success? ;T/F indicating whether their last breeding attempt was successful or unsuccessful
]

breed
[
  males male ;female birds
]
...
to go
  ...
  while [ ticks < time-to-search or any? males with [ not settled? ]]
   [
      pen-down
      find-breeding-ground
   ]

    while [ ticks < time-to-search or any? females with [ not settled? ]]
   [
      pen-down
      find-mate
   ]
...
to init-adults
  ...
  [
    ...
    set breeding-status? false ;Has not started breeding yet
  ...
  let idx 1 ;round counter

  repeat num-clust
  [
    ask n-of (count(breeders) / num-clust) turtles with [ not settled? ]
    [
      ;set breeding-ground-id to match a colony-id and that the occupancy is less than the occupancy-limit
      set breeding-ground-id idx
      set natal-ground-id idx ;intialising as the breeding-grounds = natal grounds
      ;set mating-id who ;how do I make mating pairs?
      set mating-status? true
    ]

    set idx (idx + 1); updating round counter
    ;randomly assign birds to patches
  ]
;  ask safe-patches
;  [
;   set pcolor scale-color lime occupancy 10 0
;  ]
;
;  ask pred-patches
;  [
;   set pcolor scale-color brown occupancy 10 0
;  ]
...
to init-juveniles
  ...
  repeat num-clust
  [
    ask n-of (count(recruits) / num-clust) turtles with [ not settled? ]
    [
      set breeding-ground-id 0 ;no breeding ground set
      set natal-ground-id idx ;natal grounds randomly assigned
      ;set mating-id ;how do I make mating pairs?
...
to find-breeding-ground ;a task solely for males
  ask males with [ settled? and not breeding-status?]
  [
    face patch with [ colony-id = breeding-ground-id ]
    forward 1
    ifelse patch-here with [colony-id = breeding-ground-id and occupancy < occupancy-limit ]
    [ ;the if...
      ;roll die to see if they settle
      set breeding-status? true
    ]
    [ ;the else...
  ...
  ask males with [ not settled? ]
  [
  let candidates patches with [habitable? = TRUE] in-radius detect-range

  ifelse any? candidates
  [
    face min-one-of candidates [distance myself]
    forward 1
  ]
  [
    set heading heading + random-normal 0 walk-correlation
    forward 1
  ]
  if pcolor = green and habitable? = TRUE and occupancy-limit > count turtles-here [set settled? TRUE]
      [
...
to find-mate ;a task solely for females
  ...
  ask females with [ settled? ] ;include condition that they have a mating pair?
  [
    ;face their mate and head straight there
  ]

  ask females with [ not settled? ]
  [
    ;random walk till they find a male
  ]
...
to fledging ;need to ask one in each pair to hatch....
  ask turtles with [ settled? and mating-status? ]
  [
    hatch 1
    [
