...
to go
  ...
  while [any? turtles with [ not settled? ]] ;within iteration
  [


    ask turtles with [not settled?]
    [
      pen-down
...
to init-juveniles
  ...
  [
    ...
    set ycor (50 - random 100)
    set settled? false
    set age random age-at-first-breed ;will select an integer that is between 0 and 1 year short of breeding age
    set last-breeding-success? false
    set size 3
    set life-stage "Juvenile"
  ]
...
to recruit
  ...
  [
    set age (age + 1)
    if age >= age-at-first-breed ;add some variability to this???
    [
      set life-stage "Adult"
    ]
  ]
